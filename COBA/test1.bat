...
copy "%~dp0COBA_Listes.lnk" "%appdata%\Microsoft\Windows\SendTo\"
echo.
set ok=0
if exist "%userprofile%\COBA\COBA_Listes.exe" if exist "%userprofile%\COBA\COBA_Listes.a3x" if exist "%userprofile%\COBA\COBA_Listes.ico" if exist "%desktopfolder%\COBA_Listes.lnk" if exist "%appdata%\Microsoft\Windows\SendTo\COBA_Listes.lnk" set ok=1
if !ok!==1 (
...
if not exist "%appdata%\Microsoft\Windows\SendTo\COBA_Listes.lnk" echo   - COBA_Listes.lnk ^(raccourci dans le menu "Envoyer vers..."^)
echo.
